...
class TestEdgeCases:
    def test_fewer_than_60_rows_returns_empty(self):
        result = evaluate_forecast(
            _series(30), train_size=60, test_size=20,
            horizon=1, model="gb", feature_set="default",
        )
        assert result.get("prediction_count", 0) == 0

    def test_single_row_returns_empty(self):
        result = evaluate_forecast(
            _series(1), train_size=60, test_size=20,
            horizon=1, model="gb", feature_set="default",
        )
        assert result.get("prediction_count", 0) == 0
    ...
    def test_constant_returns_produces_valid_result(self):
        result = evaluate_forecast(
            _series(200, const=True), train_size=60, test_size=20,
            horizon=1, model="gb", feature_set="vol",
        )
    ...
    def test_many_rows(self):
        result = evaluate_forecast(
            _series(500), train_size=100, test_size=20,
            horizon=1, model="gb", feature_set="default",
        )
        assert result.get("prediction_count", 0) > 0
    ...
    def test_gb_model_returns_result(self):
        result = evaluate_forecast(
            _series(200), train_size=60, test_size=20,
            horizon=1, model="gb", feature_set="default",
        )
