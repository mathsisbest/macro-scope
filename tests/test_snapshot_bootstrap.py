...
def test_fct_asset_daily_dbt_model_shape() -> None:
    ...
    dbt_model_cols = {"open", "high", "low", "volume", "ma_50", "vol_20d", "daily_return"}
    con = _snapshot_con()
    try:
...
def test_fct_asset_daily_source_is_unambiguous() -> None:
    ...
    Note: the ``source`` column was added to the dbt model after the current snapshot was
    committed.  If the column doesn't exist in the parquet yet the test is a no-op — it will
    become active once the snapshot is regenerated by the daily/weekly cron workflow."""
    con = _snapshot_con()
    try:
        df = con.execute('describe select * from marts."fct_asset_daily"').df()
    finally:
        con.close()
    actual_cols = set(df["column_name"].tolist())
    if "source" not in actual_cols:
        return  # snapshot predates the source column — no-op until regenerated
    con = _snapshot_con()
    try:
...
def test_model_metrics_has_direction_and_vol_rows() -> None:
    ...
    assert "return_gb" in models, (
        f"model_metrics is missing direction model rows (model='return_gb'). Found: {models}"
    )
