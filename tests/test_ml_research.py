...
def test_run_sweep_missing_symbol_raises():
    ...
    empty_con.execute("""
        CREATE TABLE marts.fct_asset_daily (
            symbol VARCHAR, date TIMESTAMP, open DOUBLE, high DOUBLE, low DOUBLE,
            close DOUBLE, daily_return DOUBLE
        )
    """)
    empty_con.execute(
        "CREATE TABLE marts.fct_macro_indicator (date TIMESTAMP, series_id VARCHAR, value DOUBLE)"
    )
