...
def generate_brief(con) -> str:
    ...
    if llm.available():
        try:
            ...
            raw_text, engine = llm.complete(_build_prompt(facts), system=_SYSTEM, max_tokens=4096)
...
def _gather_quant_facts(con) -> dict:
    ...
    try:
        ...
        pass
    try:
        metrics = con.execute("select model, symbol, metric, value from marts.model_metrics").df()
        facts["metrics"] = metrics.to_dict(orient="records")
    except Exception:
        pass
    return facts
...
def generate_quant_brief(con) -> pd.DataFrame:
    ...
    if llm.available():
        try:
            ...
            rejection = _validate_llm_output(raw_text)
            if rejection:
                text = "LLM output rejected: " + rejection
                engine = "offline-template (llm-rejected)"
            else:
                text = raw_text
        ...
        text = "Deterministic quant brief template (LLM unavailable)."
        engine = "offline-template"
