...
`tests/` · `.github/workflows/` (`ci.yml` — gate on PRs; `ingest.yml` — scheduled snapshot refresh) ·
`docs/` (+ ADRs).
...
- Change the `ingest.yml` cron cadence or add scheduled jobs without the owner's say-so.
