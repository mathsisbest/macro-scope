...
def render_ml_tab(rng_start, as_of):
    ...
    # Task 2.11 Move Scenario Simulator to ML Tab
    st.divider()
    st.subheader("⚡ Interactive Macro Scenario Stress-Tester")
    st.caption("Use the scenario simulator below to see how macro shocks would affect the ML-predicted returns shown above.")
    ...
    sim_fc = charts.return_forecast_table(fc)
    if not sim_fc.empty:
        _chart(
