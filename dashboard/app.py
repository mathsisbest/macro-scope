    ...
    kpis.append(
        {
            ...
            "history": spy["close"].tail(30),
        }
    )

reg = data.regimes("SPY")
if not reg.empty:
    kpis.append({"label": "SPY vol regime", "value": str(reg["regime"].iloc[-1]), "history": pd.Series(dtype=float)})
...
if not mm.empty and "yield_curve_10y_3m" in mm.columns and mm["yield_curve_10y_3m"].notna().any():
    ...
    kpis.append({"label": "10Y−3M spread", "value": f"{spread:+.2f} pp", "history": mm["yield_curve_10y_3m"].dropna().tail(30)})
elif not mm.empty and mm["yield_curve_10y_2y"].notna().any():
    spread = mm["yield_curve_10y_2y"].dropna().iloc[-1]
    kpis.append({"label": "10Y−2Y spread", "value": f"{spread:+.2f} pp", "history": mm["yield_curve_10y_2y"].dropna().tail(30)})
...
from dashboard.tabs.digest import render_digest_tab
from dashboard.tabs.markets import render_markets_tab
from dashboard.tabs.macro import render_macro_tab
from dashboard.tabs.ml_forecast import render_ml_tab
from dashboard.tabs.portfolio import render_portfolio_tab

with tab_digest:
    render_digest_tab()
with tab_mkt:
    render_markets_tab(rng_start, as_of)
with tab_macro:
    render_macro_tab(rng_start, as_of)
with tab_ml:
    render_ml_tab(rng_start, as_of)
with tab_portfolio:
    render_portfolio_tab(rng_start, as_of)
